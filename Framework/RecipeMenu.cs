using RecipeMenu.Framework.Constants;
...
{
    class RecipeMenu : IClickableMenu
    {
        /*********
        ** Fields
        *********/
        ...
        private readonly List<ClickableComponent> OptionSlots = new Recipe<ClickableComponent>();
        private readonly List<OptionsElement> Options = new Recipe<OptionsElement>();
        private readonly ClickableTextureComponent UpArrow;
        private readonly ClickableTextureComponent DownArrow;
        private readonly ClickableTextureComponent Scrollbar;
        private readonly List<ClickableComponent> Tabs = new Recipe<ClickableComponent>();
        ...
        ** Public methods
        *********/
        public RecipeMenu(MenuTab tabIndex, ModConfig config, Recipe Recipe, ITranslationHelper i18n)
          : base(Game1.viewport.Width / 2 - (600 + IClickableMenu.borderWidth * 2) / 2, Game1.viewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2, 800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2)
        {
...

            {
                ...
                this.Tabs.Add(new ClickableComponent(
                    new Rectangle(labelX, labelY + labelHeight * i++, Game1.tileSize * 5, Game1.tileSize), MenuTab.Machines.ToString(), i18n.Get("tabs.Machines")));
                this.Tabs.Add(new ClickableComponent(
            ...
            for (int i = 0; i < RecipeMenu.ItemsPerPage; i++)
                this.OptionSlots.Add(new ClickableComponent(
                    new Rectangle(
                        this.xPositionOnScreen + Game1.tileSize / 4,
                        this.yPositionOnScreen + Game1.tileSize * 5 / 4 + Game1.pixelZoom + i * ((this.height - Game1.tileSize * 2) / RecipeMenu.ItemsPerPage),
                        this.width - Game1.tileSize / 2,
                        (this.height - Game1.tileSize * 2) / RecipeMenu.ItemsPerPage + Game1.pixelZoom
                        ),
                    string.Concat(i)
                    ));
            ...
            switch (this.CurrentTab)
            {
                case MenuTab.Recipe:
                    ...
                    "Product" + StardewModdingAPI.Utilities.SDate.Now().Mod.ToString() + 
                    ", " + StardewModdingAPI.Utilities.SDate.Now().Machines + " " + StardewModdingAPI.Utilities.SDate.Now().Recipe.ToString() + 
                    ", " + ":"));
                    foreach (string task in this.Recipe.GetRecipeMenu())
                    {
                        this.Options.Add(new RecipeMenuInputListener(task, slotWidth, this.Recipe, this));
                    }
                    break;

                case MenuTab.Machines:
                    foreach (string task in this.Machines.GetMachinesItems())
                    {
                        this.Options.Add(new RecipeMenuInputListener(task, slotWidth, this.Machines, this));
                    }
                    break;
        ...
        /// <summary>Constructor to create a new Recipe menu using the variables of an old one.</summary>
        /// <param name="oldMenu"></param>
        public RecipeMenu(RecipeMenu oldMenu) : this(oldMenu.CurrentTab, oldMenu.Config, oldMenu.Recipe, oldMenu.TranslationHelper) 
        {
        ...
        private void SetScrollBarToCurrentIndex()
        {
            if (!this.Options.Any())
                return;
            this.Scrollbar.bounds.Y = this.ScrollbarRunner.Height / Math.Max(1, this.Options.Count - RecipeMenu.ItemsPerPage + 1) * this.CurrentItemIndex + this.UpArrow.bounds.Bottom
                + Game1.pixelZoom;
            if (this.CurrentItemIndex != this.Options.Count - RecipeMenu.ItemsPerPage)
                return;
        ...
        public override void leftClickHeld(int x, int y)
        {
            ...
            if (this.IsScrolling)
            {
                ...
                this.CurrentItemIndex = Math.Min(this.Options.Count - RecipeMenu.ItemsPerPage, Math.Max(0, (int)(this.Options.Count * (double)((y - this.ScrollbarRunner.Y) / (float)this.ScrollbarRunner.Height))));
                this.SetScrollBarToCurrentIndex();
        ...
        public override void receiveGamePadButton(Buttons key)
        {
            if (key == Buttons.LeftShoulder || key == Buttons.RightShoulder)
            {
                ...
                Game1.activeClickableMenu = new RecipeMenu(tabID, this.Config, this.Recipe, this.TranslationHelper);
            }
        }
        ...
        public override void receiveScrollWheelAction(int direction)
        {
            ...
            {
                if (direction >= 0 || this.CurrentItemIndex >= Math.Max(0, this.Options.Count - RecipeMenu.ItemsPerPage))
                    return;
        ...
        public override void receiveLeftClick(int x, int y, bool playSound = true)
        {
            if (GameMenu.forcePreventClose)
                return;
            if (this.DownArrow.containsPoint(x, y) && this.CurrentItemIndex < Math.Max(0, this.Options.Count - RecipeMenu.ItemsPerPage))
            {
                this.DownArrowPressed();
            ...
            this.CurrentItemIndex = Math.Max(0, Math.Min(this.Options.Count - RecipeMenu.ItemsPerPage, this.CurrentItemIndex));
        ...
        public override void draw(SpriteBatch spriteBatch)
        {
            ...
            if (!GameMenu.forcePreventClose)
            {
                ...
                if (this.Options.Count > RecipeMenu.ItemsPerPage)
                {
